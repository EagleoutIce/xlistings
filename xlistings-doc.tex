...
While it is not compatible with the \href{https://ctan.org/pkg/minted}{minted} package, it provides a similar interface for code highlighting that can be used as a (currently partial) drop-in replacement using the \argref{nofakeminted}{fakeminted} option.
...
If you are no fan of the \enquote{bordered} look of the code blocks, you can use \blatex{\\xlstsetmintedstyle\{plain number\}} to switch the look and feel to:
...
Or, using \blatex{\\xlstsetmintedstyle\{plain\}}:
